/**
 * Recreates the Inbox Wingman catalog on the LIVE Paddle account.
 * Also ensures a live client token and webhook destination.
 * Does not print secrets.
 *
 *   npx tsx scripts/seed-paddle-catalog-live.ts
 */
...
const WEBHOOK_URL = "https://www.inboxwingman.com/api/paddle/webhook";
const NAMES = {
...
function upsertEnv(name: string, value: string) {
  ...
  if (re.test(existing)) {
    writeFileSync(path, existing.replace(re, `${name}=${value}`));
    return;
  }
...
async function seed() {
  ...
  if (!apiKey.startsWith("pdl_live_")) {
    console.error("Add PADDLE_LIVE_API_KEY=pdl_live_apikey_... to .env.local. Do not paste it in chat.");
    process.exit(1);
  }
  ...
  const pilotId = await ensureProduct(
    NAMES.pilot,
    `${PLANS.pilot.credits} AI credits / month · up to ${PLANS.pilot.maxAccounts} Gmail accounts`,
    { app: APP, kind: "plan", plan: "pilot" },
  );
  ...
  let liveClientToken = readEnvLocal("NEXT_PUBLIC_PADDLE_LIVE_CLIENT_TOKEN");
  if (!liveClientToken.startsWith("live_")) {
    const token = await paddle.clientTokens.create({
      name: "Inbox Wingman live frontend",
      description: "Overlay checkout for inboxwingman.com",
    });
    liveClientToken = token.token;
    upsertEnv("NEXT_PUBLIC_PADDLE_LIVE_CLIENT_TOKEN", liveClientToken);
    console.log(`token   ${liveClientToken.slice(0, 8)}…`);
  } else {
    console.log(`token   already set (${liveClientToken.slice(0, 8)}…)`);
  }

  const destList = await paddle.notificationSettings.list({ perPage: 50 });
  const destinations = destList.map((dest) => ({
    id: dest.id,
    destination: dest.destination,
    active: dest.active,
  }));
  const webhook = destinations.find((d) => d.destination === WEBHOOK_URL);
  if (webhook) {
    console.log(`webhook exists  ${webhook.id}  active=${webhook.active}`);
  } else {
    const created = await paddle.notificationSettings.create({
      description: "Inbox Wingman production webhook",
      type: "url",
    ...
    if (created.endpointSecretKey) {
      upsertEnv("PADDLE_LIVE_NOTIFICATION_WEBHOOK_SECRET", created.endpointSecretKey);
      console.log("webhook secret written to .env.local (not printed)");
    }
  }
  ...
  const catalog = {
    ...
    clientTokenPrefix: liveClientToken.slice(0, 8),
  };
  const out = resolve(process.cwd(), ".paddle-catalog.live.json");
